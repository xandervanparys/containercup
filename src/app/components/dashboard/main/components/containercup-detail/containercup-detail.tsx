...
export default function ContainerCupDetail({ selectedCup }: cupDetailProps) {
    ...
    const [editedCup, setEditedCup] = useState<ContainerCup | null>(selectedCup);
    ...
    useEffect(() => {
        setEditedCup(selectedCup);
    }, [selectedCup]);

    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
        setEditedCup((prev) => prev ? { ...prev, [name]: value } : null);
    };

    const handleUpdate = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!user || !editedCup) {
            console.error("User is not logged in or editedCup is null");
            return;
        }
        ...
        try {
            await updateContainerCup(editedCup.id, {
                name: editedCup.name,
                description: editedCup.description,
                image_url: imageUrl,
            });
            console.log("Successfully edited:", editedCup.name);
            setIsEditing(false);
        } catch (error) {
    ...
    const removeCup = async () => {
        if (selectedCup) {
            try {
                await deleteContainerCup(selectedCup.id);
                console.log("Removed cup:", selectedCup.name);
            } catch (error) {
                console.error("Error removing container cup:", error);
            }
        }
    }
    ...
    const cestDateTime = selectedCup.updated_at ? new Date(selectedCup.updated_at) : new Date();
    const updateDate: string = cestDateTime.toDateString();
    const updateTime: string = `${cestDateTime.getHours()}:${cestDateTime.getMinutes()}:${cestDateTime.getSeconds()}`;
